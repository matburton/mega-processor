...
internal sealed class Calculate(Reference m_MainGlobals)
{
    ...
    private Assembly CalculateRoutine(Assembly a) => a
        ...
        .Compare(R0, R1)
        .GoToIf(C.LessOrEqual, out var wasNotInCheck)
        ...
        .GoToIf(C.Equal, out var winGameValueDepthZero)
        .SetWordValue(R3, -512)
        .SetByteValue(R2, 1)
        .Compare(R0, R2)
        .Append((winGameValueDepthOne, a) => a
        ...
        .Compare(R0, R2)
        .Append((singlePawnJumpBlack, a) => a
        ...
        .GoToIf(C.NotEqual, out var notDepth0AndMode1)
        ...
        .Append((checkDepthAndModeForInitialOriginSquareIndex, a) => a
            .GoTo(checkDepthAndModeForInitialOriginSquareIndex, forceAbsolute: true)
            .DefineReference(notDepth0AndMode1)
            .SetByteValue(R0, 21))
        .CopyByteToStack(m_Stuff.Locals.OriginSquareIndex, R0)
        .CopyByteFromStack(m_Stuff.Args.ModeMaxDepth, R2)
        .GoToIf(C.Equal, out var clearProgressMode0)
        .CopyByteFromStack(m_Stuff.Args.Depth, R2)
        .Append((clearProgressNotDepth0, a) => a
            .GoToIf(C.NotEqual, clearProgressNotDepth0)
            .SetWordValue(R2, 0xA0E0)
            .SetByteValue(R1, 0, force: true)
            .Repeat(4, (_, a) => a.CopyWordToIndex(R2, R1, bumpIndex: true)))
        .SetByteValue(R1, 1)
        .Compare(R2, R1)
        .Append((clearProgressNotDepth1, a) => a
            .GoToIf(C.NotEqual, clearProgressNotDepth1)
            .SetWordValue(R1, 0xA0E8)
            .SetByteValue(R1, 0, force: true)
            .Repeat(4, (_, a) => a.CopyWordToIndex(R2, R1, bumpIndex: true)))
        .SetWordValue(R2, 0xA0F0)
        .SetByteValue(R1, 0, force: true)
        .Repeat(4, (_, a) => a.CopyWordToIndex(R2, R1, bumpIndex: true))
        .DefineReference(clearProgressMode0)
        .SetWordValue(R2, 0xA0F8)
        .SetByteValue(R1, 0, force: true)
        .Repeat(4, (_, a) => a.CopyWordToIndex(R2, R1, bumpIndex: true))
        .DefineReference(out var forLoopStart)
        .SetWordValue(R2, m_BoardState, force: true)
        .AddTo(R2, R0)
        .CopyByteFromIndex(R2, R0)
        .CopyByteToStack(m_Stuff.Locals.OriginSquareValue, R0)
        .DeclareReference(out var handleOriginPieceBlockEnd)
        .Append((originSquareValueNotEmpty, a) => a
            .GoToIf(C.NotEqual, originSquareValueNotEmpty)
            .GoTo(handleOriginPieceBlockEnd, forceAbsolute: true))
        .SetByteValue(R1, Piece.Enum.OffBoard)
        .Compare(R0, R1)
        .Append((originSquareValueNotOffBoard, a) => a
            .GoToIf(C.NotEqual, originSquareValueNotOffBoard)
            .GoTo(handleOriginPieceBlockEnd, forceAbsolute: true))
        .SetByteValue(R1, Piece.Colour.Mask)
        .AndTo(R1, R0)
        .CopyByteFromStack(m_Stuff.Locals.OriginPieceColor, R2)
        .Compare(R1, R2)
        .Append((originSquareValueNotOriginColor, a) => a
            .GoToIf(C.Equal, originSquareValueNotOriginColor)
            .GoTo(handleOriginPieceBlockEnd, forceAbsolute: true))
        .CopyByteFromStack(m_Stuff.Args.ModeMaxDepth, R2)
        .DeclareReference(out var increaseProgress)
        .Append((increaseProgressNotMode0, a) => a
            .GoToIf(C.NotEqual, increaseProgressNotMode0)
            .SetWordValue(R2, 0xA0F8)
            .GoTo(increaseProgress, forceAbsolute: true))
        .CopyByteFromStack(m_Stuff.Args.Depth, R2)
        .Append((increaseProgressNotDepth0, a) => a
            .GoToIf(C.NotEqual, increaseProgressNotDepth0)
            .SetWordValue(R2, 0xA0E0)
            .GoTo(increaseProgress, forceAbsolute: true))
        .SetByteValue(R1, 1)
        .Compare(R2, R1)
        .Append((increaseProgressNotDepth1, a) => a
            .GoToIf(C.NotEqual, increaseProgressNotDepth1)
            .SetWordValue(R2, 0xA0E8)
            .GoTo(increaseProgress, forceAbsolute: true))
        .SetWordValue(R2, 0xA0F0)
        .DefineReference(increaseProgress)
        .SetWordValue(R3, 0xFFFF)
        .CopyWordFromIndex(R2, R1, bumpIndex: true)
        .Compare(R3, R1)
        .Append((increaseProgressDone, a) => a
            .GoToIf(C.NotEqual, increaseProgressDone)
            .CopyWordFromIndex(R2, R1, bumpIndex: true)
            .Compare(R3, R1)
            .GoToIf(C.NotEqual, increaseProgressDone)
            .CopyWordFromIndex(R2, R1, bumpIndex: true)
            ...
            .CopyWordFromIndex(R2, R1, bumpIndex: true))
        .AddConst(R2, -2)
        .Repeat(times: 4, (_, a) => a.AddTo(R1, R1))
        .SetWordValue(R3, 15, force: true)
        .AddTo(R1, R3)
        .CopyWordToIndex(R2, R1)
        .SetByteValue(R1, Piece.ValueMask)
        .AndTo(R1, R0)
        .CopyByteToStack(m_Stuff.Locals.MovedOriginPieceValue, R1)
        .CopyByteFromStack(m_Stuff.Locals.OriginPieceColor, R2)
        .ExclusiveOrTo(R2, R1)
        .CopyByteToStack(m_Stuff.Locals.ColorlessOriginPieceValue, R2)
        ...
        .Loop((moreMovesLoop, a) => a
            .CopyByteFromStack(m_Stuff.Locals.TargetSquareIndex, R0)
            .CopyWordFromStack(m_Stuff.Locals.MoveDirectionIndex, R2)
            .CopyWordFromIndex(R2, R1)
            .AddTo(R0, R1)
            .CopyByteToStack(m_Stuff.Locals.TargetSquareIndex, R0)
            .SetWordValue(R2, m_BoardState, force: true)
            .AddTo(R2, R0)
            .CopyByteFromIndex(R2, R1)
            .CopyByteToStack(m_Stuff.Locals.TargetSquareValue, R1)
            .SetByteValue(R3, 0, force: true)
            .CopyByteToStack(m_Stuff.Locals.OtherSquareOriginIndex, R3)
            .CopyByteToStack(m_Stuff.Locals.OtherSquareTargetIndex, R3)
            .Test(R1)
            .GoToIf(C.NotEqual, out var targetNotEmpty)
            .CopyByteFromStack(m_Stuff.Locals.OriginPieceIsAPawn, R2)
            .GoToIf(C.NotEqual, out var emptyTargetOriginPawn)
            .GoTo(out var originCanMoveToTargetBlockStart, forceAbsolute: true)
            .DefineReference(emptyTargetOriginPawn)
            .CopyWordFromStack(m_Stuff.Locals.SinglePawnJump, R2)
            .CopyTo(R3, R0)
            .SubFrom(R3, R2)
            .CopyByteFromStack(m_Stuff.Args.EnPassantPawnIndex, R2)
            .Compare(R3, R2)
            .Append((cannotTakeEnPassant, a) => a
                .GoToIf(C.NotEqual, cannotTakeEnPassant)
                .CopyByteToStack(m_Stuff.Locals.OtherSquareOriginIndex, R2)
                .CopyByteToStack(m_Stuff.Locals.OtherSquareTargetIndex, R2)
                .GoTo(originCanMoveToTargetBlockStart, forceAbsolute: true))
